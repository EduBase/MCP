...
/* Certificate settings of an exam, returned by every /exam:certificates endpoint (only the state is returned when the certificates are not enabled, and the settings of the items that are not displayed are omitted) */
const EXAM_CERTIFICATES_OUTPUT_SCHEMA = z.object({
	exam: z.string().describe('exam identification string'),
	enabled: z.boolean().describe('certificates are issued for the exam, the configuration itself is only returned when the certificates are enabled'),
	configured: z.boolean().describe('the certificates are configured, so they can be issued'),
	supervisors_only: z.boolean().optional().describe('only the supervisors can issue the certificates, the examinees cannot get their own ones'),
	email_send: z.boolean().optional().describe('the certificates are sent to the examinees in email automatically'),
	preset: z.string().nullable().optional().describe('identification string of the certificate preset configuring the certificates (see edubase_get_exam_certificates_presets), null if the certificates are configured manually'),
	language: z.string().optional().describe('language of the certificates'),
	title: z.string().optional().describe('title of the certificates'),
	text1: z.string().optional().describe('text displayed under the data of the examinee, the multi line texts (text1, text2 and notes) are returned with basic HTML formatting and line breaks'),
	text2: z.string().optional().describe('text displayed under the results, above the date of the test'),
	text_date: z.string().optional().describe('text displayed before the date of the test'),
	notes: z.string().optional().describe('notes displayed at the bottom of the certificates'),
	show_result: z.boolean().optional().describe('the result of the test is displayed'),
	show_serial: z.boolean().optional().describe('the certificates get a serial number'),
	show_birthdate_birthplace: z.boolean().optional().describe('the birth date and the birth place of the examinee are displayed'),
	show_custom: z.boolean().optional().describe('custom user data is displayed'),
	show_expiration: z.boolean().optional().describe('the certificates expire and their validity is displayed'),
	show_signature: z.boolean().optional().describe('signatures are displayed'),
	show_custom_branding: z.boolean().optional().describe('custom branding (logo and organization name) is displayed'),
	hide_timezone: z.boolean().optional().describe('the timezone information is hidden'),
	download_automatically: z.boolean().optional().describe('the certificate is downloaded automatically when the examinee opens the results page'),
	grading_threshold: z.number().nullable().optional().describe('threshold of the certificates in percentage, overriding the threshold of the grading, null if the threshold of the grading is used'),
	timezone: z.string().nullable().optional().describe('timezone of the dates on the certificates, null if the timezone of the exam is used'),
	custom_fields: z.array(z.string()).optional().describe('custom user data displayed as extra data (only present if show_custom is enabled), the custom user data fields of the exam are identified by their label (see edubase_get_exam_fields), the data fields of the user accounts by their customer_ prefixed name'),
	custom_user_fields: z.array(z.string()).optional().describe('custom user data displayed together with the data of the examinee (only present if show_custom is enabled)'),
	result_format: z.enum(['points-percent-time', 'points-percent', 'points', 'percent']).optional().describe('format of the displayed result (only present if show_result is enabled)'),
	serial_format: z.enum(['prefix-year-index', 'prefix-date-index', 'prefix-index']).nullable().optional().describe('format of the serial numbers (only present if show_serial is enabled)'),
	serial_separator: z.string().optional().describe('separator between the parts of the serial numbers (only present if show_serial is enabled)'),
	serial_prefix: z.string().optional().describe('prefix of the serial numbers (only present if show_serial is enabled)'),
	serial_index_digits: z.number().int().optional().describe('minimum length of the index in the serial numbers (only present if show_serial is enabled)'),
	expiration: z.number().int().optional().describe('validity period of the certificates in days (only present if show_expiration is enabled)'),
	renew: z.union([
		z.boolean(),
		z.number().int(),
	]).optional().describe('whether the certificates can be renewed (only present if show_expiration is enabled), false: the certificates cannot be renewed, true: the certificates can be renewed any time, number: the number of days the renewal is available from, relative to the expiry (0 is the day of the expiry, negative values open the renewal before it)'),
	manual_renewal: z.boolean().optional().describe('the renewal has to be approved by a supervisor (only present if show_expiration is enabled)'),
	automatic_renewal_approval: z.boolean().optional().describe('the renewal is approved automatically, but it stays revocable (only present if show_expiration is enabled)'),
	expiration_notification_emails: z.array(z.string()).optional().describe('email addresses notified about the expiring certificates (only present if show_expiration is enabled)'),
	signature_user: z.boolean().optional().describe('a signature line is displayed for the examinee (only present if show_signature is enabled)'),
	signature_left: z.boolean().optional().describe('the signature of the first organizer is displayed (only present if show_signature is enabled)'),
	signature_left_line1: z.string().optional().describe('name of the first organizer (only present if show_signature is enabled)'),
	signature_left_line2: z.string().optional().describe('second line of the first signature (only present if show_signature is enabled)'),
	signature_left_line3: z.string().optional().describe('third line of the first signature (only present if show_signature is enabled)'),
	signature_left_line4: z.string().optional().describe('fourth line of the first signature (only present if show_signature is enabled)'),
	signature_left_image: z.boolean().optional().describe('the uploaded image of the first signature is displayed (only present if show_signature is enabled)'),
	signature_right: z.boolean().optional().describe('the signature of the second organizer is displayed (only present if show_signature is enabled)'),
	signature_right_line1: z.string().optional().describe('name of the second organizer (only present if show_signature is enabled)'),
	signature_right_line2: z.string().optional().describe('second line of the second signature (only present if show_signature is enabled)'),
	signature_right_line3: z.string().optional().describe('third line of the second signature (only present if show_signature is enabled)'),
	signature_right_line4: z.string().optional().describe('fourth line of the second signature (only present if show_signature is enabled)'),
	signature_right_image: z.boolean().optional().describe('the uploaded image of the second signature is displayed (only present if show_signature is enabled)'),
	signature_stamp: z.boolean().optional().describe('the uploaded stamp is displayed between the signatures (only present if show_signature is enabled)'),
	branding_line1: z.string().optional().describe('name of the organization (only present if show_custom_branding is enabled)'),
	branding_line2: z.string().optional().describe('second line of the branding (only present if show_custom_branding is enabled)'),
	branding_line3: z.string().optional().describe('third line of the branding (only present if show_custom_branding is enabled)'),
	branding_line4: z.string().optional().describe('fourth line of the branding (only present if show_custom_branding is enabled)'),
	branding_image: z.boolean().optional().describe('the uploaded logo is displayed (only present if show_custom_branding is enabled)'),
	branding_edubase: z.boolean().optional().describe('the EduBase logo is displayed (only present if show_custom_branding is enabled)'),
});
...
export const EDUBASE_API_TOOLS_EXAMS = [
	...
	// POST /exam:users:generate - Generate exam accounts from patterns and assign them to an exam
	{
		...
		outputSchema: z.object({
			...
			failed: z.array(z.string()).optional().describe('usernames of the accounts that could not be created (only present if there are any)'),
			duplicate: z.array(z.string()).optional().describe('usernames that were skipped because the same account was being generated by a parallel request at the same time (only present if there are any)'),
		}),
	},
	...
	// POST /exam:certificates - Change individual certificate settings of an exam
	{
		...
		inputSchema: z.object({
			exam: z.string().describe('exam identification string'),
			enabled: z.boolean().optional().describe('issue certificates for the exam, needs the certificate feature to be enabled for the user, not available for surveys, and needs a grading that decides whether the test was successful (see the certificates flag in edubase_get_quiz_grading_presets), the configuration is kept when the certificates are disabled'),
			supervisors_only: z.boolean().optional().describe('only allow the supervisors to issue the certificates, so the examinees cannot get their own ones'),
			email_send: z.boolean().optional().describe('send the certificates to the examinees in email automatically, needs the certificate email sending feature to be enabled on the instance'),
			language: z.string().optional().describe('language of the certificates, only the languages EduBase is translated to are available (default: language of the exam)'),
			title: z.string().min(1).max(80).optional().describe('title of the certificates, certificates are only issued once they have a title'),
			text1: z.string().max(320).optional().describe('text displayed under the data of the examinee, the multi line texts (text1, text2 and notes) support basic formatting (bold, italic, underline, subscript, superscript) and line breaks'),
			text2: z.string().max(1280).optional().describe('text displayed under the results, above the date of the test'),
			text_date: z.string().max(80).optional().describe('text displayed before the date of the test'),
			notes: z.string().max(320).optional().describe('notes displayed at the bottom of the certificates'),
			show_result: z.boolean().optional().describe('display the result of the test'),
			show_serial: z.boolean().optional().describe('give a serial number to the certificates'),
			show_birthdate_birthplace: z.boolean().optional().describe('display the birth date and the birth place of the examinee'),
			show_custom: z.boolean().optional().describe('display custom user data'),
			show_expiration: z.boolean().optional().describe('expire the certificates and display their validity'),
			show_signature: z.boolean().optional().describe('display signatures'),
			show_custom_branding: z.boolean().optional().describe('display custom branding (logo and organization name)'),
			hide_timezone: z.boolean().optional().describe('hide the timezone information'),
			download_automatically: z.boolean().optional().describe('download the certificate automatically when the examinee opens the results page'),
			grading_threshold: z.union([
				z.number().min(0).max(100),
				z.string().regex(/^(\d+(\.\d+)?|default)$/i),
			]).optional().describe('threshold of the certificates in percentage, overriding the threshold of the grading, certificates are only issued above it. Accepts a number or a numeric string between 0 and 100, or "default" if the threshold of the grading should be used'),
			timezone: z.string().optional().describe('timezone of the dates on the certificates, or "default" if the timezone of the exam should be used'),
			custom_fields: z.array(z.string()).optional().describe('custom user data to display as extra data, requires show_custom, at most three fields can be displayed, the custom user data fields of the exam are identified by their label (see edubase_get_exam_fields), the data fields of the user accounts by their customer_ prefixed name, send an empty list to remove every field'),
			custom_user_fields: z.array(z.string()).optional().describe('custom user data to display together with the data of the examinee, the same way as with custom_fields'),
			result_format: z.enum(['points-percent-time', 'points-percent', 'points', 'percent']).optional().describe('format of the displayed result, requires show_result (points-percent-time: points, result in percentage and the time spent with the test, points-percent: points and result in percentage, points: points only, percent: result in percentage only), default: points-percent-time'),
			serial_format: z.enum(['prefix-year-index', 'prefix-date-index', 'prefix-index']).optional().describe('format of the serial numbers, requires show_serial and it is mandatory when the serial numbers are configured for the first time (prefix-year-index: prefix, the year of the test and the index, prefix-date-index: prefix, the date of the test and the index, prefix-index: prefix and index, written together), none of the serial_ settings can be changed anymore once a certificate is issued for the exam'),
			serial_separator: z.enum(['/', '-', '']).optional().describe('separator between the parts of the serial numbers, requires show_serial, always empty with the prefix-index format (default: /)'),
			serial_prefix: z.string().max(16).optional().describe('prefix of the serial numbers, requires show_serial, the serial numbers are only unique within the exam, so a unique prefix is recommended'),
			serial_index_digits: z.number().int().min(1).max(16).optional().describe('minimum length of the index in the serial numbers, between 1 and 16, requires show_serial (default: 4)'),
			expiration: z.number().int().min(0).max(9999).optional().describe('validity period of the certificates in days, between 0 and 9999, requires show_expiration and it is mandatory when the expiration is configured for the first time'),
			renew: z.union([
				z.boolean(),
				z.number().int(),
			]).optional().describe('whether the certificates can be renewed, requires show_expiration (false: the certificates cannot be renewed, true: the certificates can be renewed any time, number: the number of days the renewal is available from, relative to the expiry, 0 is the day of the expiry, negative values open the renewal before it, at most the validity period earlier), default: false'),
			manual_renewal: z.boolean().optional().describe('the renewal has to be approved by a supervisor, requires renew'),
			automatic_renewal_approval: z.boolean().optional().describe('approve the renewal automatically, but keep it revocable, requires manual_renewal'),
			expiration_notification_emails: z.array(z.string()).optional().describe('email addresses to notify about the expiring certificates, requires show_expiration, the examinees of the exam are notified automatically about their renewable certificates, these notifications are sent to the specified addresses only, send an empty list to remove every address'),
			signature_user: z.boolean().optional().describe('display a signature line for the examinee, requires show_signature'),
			signature_left: z.boolean().optional().describe('display the signature of the first organizer, requires show_signature, the lines of the signature are kept when it is disabled'),
			signature_left_line1: z.string().min(1).max(40).optional().describe('name of the first organizer, mandatory when the signature is displayed'),
			signature_left_line2: z.string().max(50).optional().describe('second line of the first signature'),
			signature_left_line3: z.string().max(50).optional().describe('third line of the first signature'),
			signature_left_line4: z.string().max(50).optional().describe('fourth line of the first signature'),
			signature_left_image: z.union([
				z.boolean(),
				z.string(),
			]).optional().describe('image of the first signature, the identifier of an image uploaded to the filebin replaces the image (supported formats: JPEG, WebP, PNG), the white background of the signatures and the stamp is removed automatically, but transparent PNG images are recommended (true: display the image that is already uploaded, false: keep the uploaded image, but do not display it)'),
			signature_right: z.boolean().optional().describe('display the signature of the second organizer, the same way as with signature_left'),
			signature_right_line1: z.string().min(1).max(40).optional().describe('name of the second organizer, mandatory when the signature is displayed'),
			signature_right_line2: z.string().max(50).optional().describe('second line of the second signature'),
			signature_right_line3: z.string().max(50).optional().describe('third line of the second signature'),
			signature_right_line4: z.string().max(50).optional().describe('fourth line of the second signature'),
			signature_right_image: z.union([
				z.boolean(),
				z.string(),
			]).optional().describe('image of the second signature, the same way as with signature_left_image'),
			signature_stamp: z.union([
				z.boolean(),
				z.string(),
			]).optional().describe('stamp displayed between the signatures, the same way as with signature_left_image'),
			branding_line1: z.string().min(1).max(80).optional().describe('name of the organization, requires show_custom_branding and it is mandatory when the branding is displayed'),
			branding_line2: z.string().max(80).optional().describe('second line of the branding'),
			branding_line3: z.string().max(80).optional().describe('third line of the branding'),
			branding_line4: z.string().max(80).optional().describe('fourth line of the branding'),
			branding_image: z.union([
				z.boolean(),
				z.string(),
			]).optional().describe('logo of the branding, the same way as with signature_left_image'),
			branding_edubase: z.boolean().optional().describe('display the EduBase logo, it can only be removed with the matching feature enabled'),
		}),
		outputSchema: EXAM_CERTIFICATES_OUTPUT_SCHEMA,
	},

	// PUT /exam:certificates - Replace the complete certificate configuration of an exam
	{
		name: 'edubase_put_exam_certificates',
		description: "Replace the complete certificate configuration of an exam with a preset or with the configuration of another exam. The certificates have to be enabled for the exam, and its settings cannot be changed while it is locked.",
		inputSchema: z.object({
			exam: z.string().describe('exam identification string (the target exam)'),
			preset: z.string().optional().describe('certificate preset identification string (see edubase_get_exam_certificates_presets), the settings of the preset are applied to the exam, and they are kept up to date with the preset'),
			source: z.string().optional().describe('exam identification string to copy the certificate configuration from, must be different from the target exam, and its certificates have to be configured manually (not by a preset), the images (signatures, stamp and logo) are copied as well'),
		}),
		outputSchema: EXAM_CERTIFICATES_OUTPUT_SCHEMA,
	},

	// DELETE /exam:certificates - Stop issuing certificates for an exam
	{
		name: 'edubase_delete_exam_certificates',
		description: "Stop issuing certificates for an exam. The configuration itself is kept, so the certificates can be enabled again later with edubase_post_exam_certificates. The already issued certificates are not revoked. The settings cannot be changed while the exam is locked.",
		inputSchema: z.object({
			exam: z.string().describe('exam identification string'),
		}),
		outputSchema: EXAM_CERTIFICATES_OUTPUT_SCHEMA,
	...
	// GET /exam:certificates:presets - List the certificate presets that can be used on exams
	{
		name: 'edubase_get_exam_certificates_presets',
		description: "List the certificate presets that can be used on exams. The public presets, the owned presets and the presets of the organizations of the user are returned.",
		inputSchema: z.object({
			exam: z.string().optional().describe('exam identification string, only the presets that can be used on this exam are listed, the language of the exam is used for the ordering of the presets'),
			language: z.string().optional().describe('optional language to filter the results with'),
		}),
		outputSchema: z.object({
			exam: z.string().optional().describe('exam identification string (only present if an exam is specified)'),
			presets: z.array(z.object({
				preset: z.string().describe('certificate preset identification string'),
				title: z.string().describe('title of the certificate preset'),
				language: z.string().describe('language of the certificate preset'),
				public: z.boolean().describe('the preset is a public preset available for everyone'),
				...
				current: z.boolean().describe('the certificates of the exam are configured by this preset'),
			})),
		}),
	},
