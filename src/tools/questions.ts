...
export const EDUBASE_API_TOOLS_QUESTIONS = [
	...
	// POST /question - Publish or update a question
	{
		...
		inputSchema: z.object({
				...
				type: z.enum(['GENERIC', 'TEXT', 'FREE-TEXT', 'READING', 'CHOICE', 'MULTIPLE-CHOICE', 'ORDER', 'TRUE/FALSE', 'GROUPING', 'PAIRING', 'NUMERIC', 'DATE/TIME', 'EXPRESSION', 'MATRIX', 'MATRIX:EXPRESSION', 'SET', 'SET:TEXT', 'HOTSPOT', 'FILE']).describe('Type of the question.'),
				...
				image: z.string().describe('Attach an image to the question. Supported formats: PNG, JPEG, WebP. Format: filename=data, where data is either a base64-encoded image (whole data string) or a URL.'),
				...
				solution_image: z.string().describe('Attach an image to the solution steps. Supported formats: PNG, JPEG, WebP. Format: filename=data, where data is either a base64-encoded image (whole data string) or a URL.'),
				...
				attachment: z.string().describe('Attach a file to the question. Format: filename=data, where data is either a base64-encoded file (whole data string) or a URL.'),
				media_audio: z.string().describe('Attach an audio file to the question. Supported formats: MP3, AAC, M4A. Format: filename=data, where data is either a base64-encoded audio file (whole data string) or a URL.'),
				...
				hotspot_image: z.string().describe('The image on which the points must be marked. Applicable only for HOTSPOT questions. Supported formats: PNG, JPEG, WebP. Format: filename=data, where data is either a base64-encoded image (whole data string) or a URL.'),
				...
				tags: z.string().describe('Tag questions with custom user-defined tags. Use ID or code of pre-registered tags.'),
				labels: z.string().describe('Categorize questions with instance-level labels. Pre-defined values specific to each EduBase instance.'),
