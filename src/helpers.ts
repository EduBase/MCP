...
export function stripSchemaDescriptions<T>(schema: T): T {
	...
	switch (def.type) {
		case 'object':
			...
			if (def.catchall) def.catchall = stripSchemaDescriptions(def.catchall);
			break;
		case 'array':
			def.element = stripSchemaDescriptions(def.element);
			break;
		case 'union':
			def.options = def.options.map((option: unknown) => stripSchemaDescriptions(option));
			break;
		case 'intersection':
		...
		case 'record':
			...
			break;
		case 'tuple':
			def.items = def.items.map((item: unknown) => stripSchemaDescriptions(item));
			if (def.rest) def.rest = stripSchemaDescriptions(def.rest);
			break;
		case 'pipe':
		...
		default:
			...
			if (def.innerType) def.innerType = stripSchemaDescriptions(def.innerType);
	}
	/* Cloning creates a new instance, which is not registered with the description in the global registry */
	return (schema as unknown as { clone: (def: Record<string, any>) => T }).clone(def);
}
