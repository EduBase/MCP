...
function isDisallowedIpv6(ip) {
    ...
    if (zone !== -1)
        addr = addr.slice(0, zone); /* strip scope id (fe80::1%eth0) */
...
async function resolveToValidatedIp(hostname) {
    ...
    if (records.length === 0)
        throw new Error(`No DNS records for host: ${hostname}`);
...
async function buildPinnedDispatcher(parsed) {
...
export async function guardedRequest(url, options = {}, enforceSsrfProtection = false) {
...
export function parseFileUri(source) {
...
export async function fetchFileFromUrl(url, enforceSsrfProtection = false) {
...
export async function getFileBuffer(source, options = {}) {
...
export function expandCustomFields(args) {
